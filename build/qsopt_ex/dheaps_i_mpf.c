...
void mpf_ILLutil_dheap_findmin (
	mpf_ILLdheap * h,
	int *i)
{
	if (h->size == 0)
...
int mpf_ILLutil_dheap_insert (
	mpf_ILLdheap * h,
	int i)
{
	if (h->size >= h->total_space)
	{
		QSlog("Error - mpf_heap already full");
		return 1;
	}
	h->size++;
...
void mpf_ILLutil_dheap_deletemin (
	mpf_ILLdheap * h,
	int *i)
{
	int j;
...
void mpf_ILLutil_dheap_changekey (
	mpf_ILLdheap * h,
	int i,
	mpf_t * newkey)
{
