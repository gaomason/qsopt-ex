    ...
    mpf_svector * spike)
{
    ...
    else
    {
        ...
        for (i = 0; i < f->dim; i++)
        {
            ...
            mpf_EGlpNumZero(work_coef[i]); // Clean up
        }
        spike->nzcnt = nzcnt;
    }
	return 0;
...
/**
 * @brief Creates a deep copy of an mpf_factor_work struct.
 *
 ...
 */
 int mpf_ILLfactor_deep_copy(
    mpf_factor_work *dest,
    const mpf_factor_work *src)
{
    int i;
    int rval = 0;
    int dsize;
    ...
    mpf_EGlpNumCopy(dest->fzero_tol, src->fzero_tol);
    mpf_EGlpNumCopy(dest->szero_tol, src->szero_tol);
    mpf_EGlpNumCopy(dest->partial_tol, src->partial_tol);
    ...
    mpf_EGlpNumCopy(dest->maxelem_orig, src->maxelem_orig);
    dest->nzcnt_orig = src->nzcnt_orig;
    mpf_EGlpNumCopy(dest->maxelem_factor, src->maxelem_factor);
    dest->nzcnt_factor = src->nzcnt_factor;
    mpf_EGlpNumCopy(dest->maxelem_cur, src->maxelem_cur);
    dest->nzcnt_cur = src->nzcnt_cur;
    mpf_EGlpNumCopy(dest->partial_cur, src->partial_cur);
    dest->dim = src->dim;
    ...
    if (src->work_coef) {
        ...
        for (i = 0; i < src->dim; i++) {
            mpf_EGlpNumCopy(dest->work_coef[i], src->work_coef[i]);
        }
    }
    ...
    if (src->uc_inf) {
        ...
        memcpy(dest->uc_inf, src->uc_inf, (src->dim + src->max_k + 1) * sizeof(mpf_uc_info));
    }
    if (src->ur_inf) {
        ILL_SAFE_MALLOC(dest->ur_inf, src->dim + src->max_k + 1, mpf_ur_info);
        memcpy(dest->ur_inf, src->ur_inf, (src->dim + src->max_k + 1) * sizeof(mpf_ur_info));
        for (i = 0; i < src->dim + src->max_k + 1; i++) {
            mpf_EGlpNumInitVar(dest->ur_inf[i].max);
            mpf_EGlpNumCopy(dest->ur_inf[i].max, src->ur_inf[i].max);
        }
    }
    if (src->lc_inf) {
        ILL_SAFE_MALLOC(dest->lc_inf, src->dim, mpf_lc_info);
        memcpy(dest->lc_inf, src->lc_inf, src->dim * sizeof(mpf_lc_info));
    }
    if (src->lr_inf) {
        ILL_SAFE_MALLOC(dest->lr_inf, src->dim, mpf_lr_info);
        memcpy(dest->lr_inf, src->lr_inf, src->dim * sizeof(mpf_lr_info));
    }
    if (src->er_inf) {
        ILL_SAFE_MALLOC(dest->er_inf, src->etamax, mpf_er_info);
        memcpy(dest->er_inf, src->er_inf, src->etamax * sizeof(mpf_er_info));
    }
    ...
    if (src->uccoef) {
        ...
        for (i = 0; i < src->uc_space; i++) {
            mpf_EGlpNumCopy(dest->uccoef[i], src->uccoef[i]);
        }
    }
    ...
    if (src->urcoef) {
        ...
        for (i = 0; i < src->ur_space; i++) {
            mpf_EGlpNumCopy(dest->urcoef[i], src->urcoef[i]);
        }
    }
    ...
    if (src->lccoef) {
        ...
        for (i = 0; i < src->lc_space; i++) {
            mpf_EGlpNumCopy(dest->lccoef[i], src->lccoef[i]);
        }
    }
	...
	if (src->lrcoef) {
		...
		for (i = 0; i < lr_nzcnt; i++) {
			mpf_EGlpNumCopy(dest->lrcoef[i], src->lrcoef[i]);
		}
	}
    ...
    if (src->ercoef) {
        ...
        for (i = 0; i < src->er_space; i++) {
            mpf_EGlpNumCopy(dest->ercoef[i], src->ercoef[i]);
        }
    }
    ...
    if (src->dmat) {
        ...
        for (i = 0; i < dsize; i++) {
            mpf_EGlpNumCopy(dest->dmat[i], src->dmat[i]);
        }
    }
    ...
    if (src->xtmp.nzcnt > 0) {
        ...
        for (i = 0; i < src->xtmp.nzcnt; i++) {
            mpf_EGlpNumCopy(dest->xtmp.coef[i], src->xtmp.coef[i]);
        }
    }
