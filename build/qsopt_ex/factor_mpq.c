    ...
    mpq_svector * spike)
{
    ...
    else
    {
        ...
        for (i = 0; i < f->dim; i++)
        {
            ...
            mpq_EGlpNumZero(work_coef[i]); // Clean up
        }
        spike->nzcnt = nzcnt;
    }
	return 0;
...
/**
 * @brief Creates a deep copy of an mpq_factor_work struct.
 *
 ...
 */
 int mpq_ILLfactor_deep_copy(
    mpq_factor_work *dest,
    const mpq_factor_work *src)
{
    int i;
    int rval = 0;
    int dsize;
    ...
    mpq_ILLfactor_init_factor_work(dest);
    // Copy scalar fields from src to dest.
    dest->max_k = src->max_k;
    mpq_EGlpNumCopy(dest->fzero_tol, src->fzero_tol);
    mpq_EGlpNumCopy(dest->szero_tol, src->szero_tol);
    mpq_EGlpNumCopy(dest->partial_tol, src->partial_tol);
    ...
    mpq_EGlpNumCopy(dest->maxelem_orig, src->maxelem_orig);
    dest->nzcnt_orig = src->nzcnt_orig;
    mpq_EGlpNumCopy(dest->maxelem_factor, src->maxelem_factor);
    dest->nzcnt_factor = src->nzcnt_factor;
    mpq_EGlpNumCopy(dest->maxelem_cur, src->maxelem_cur);
    dest->nzcnt_cur = src->nzcnt_cur;
    mpq_EGlpNumCopy(dest->partial_cur, src->partial_cur);
    dest->dim = src->dim;
    ...
    if (src->work_coef) {
        dest->work_coef = mpq_EGlpNumAllocArray(src->dim);
        if (!dest->work_coef) { rval = 1; goto CLEANUP; }
        for (i = 0; i < src->dim; i++) {
            mpq_EGlpNumCopy(dest->work_coef[i], src->work_coef[i]);
        }
    }
    ...
    // Info structs
    if (src->uc_inf) {
        ILL_SAFE_MALLOC(dest->uc_inf, src->dim + src->max_k + 1, mpq_uc_info);
        memcpy(dest->uc_inf, src->uc_inf, (src->dim + src->max_k + 1) * sizeof(mpq_uc_info));
    }
    if (src->ur_inf) {
        ILL_SAFE_MALLOC(dest->ur_inf, src->dim + src->max_k + 1, mpq_ur_info);
        memcpy(dest->ur_inf, src->ur_inf, (src->dim + src->max_k + 1) * sizeof(mpq_ur_info));
        for (i = 0; i < src->dim + src->max_k + 1; i++) {
            mpq_EGlpNumInitVar(dest->ur_inf[i].max);
            mpq_EGlpNumCopy(dest->ur_inf[i].max, src->ur_inf[i].max);
        }
    }
    if (src->lc_inf) {
        ILL_SAFE_MALLOC(dest->lc_inf, src->dim, mpq_lc_info);
        memcpy(dest->lc_inf, src->lc_inf, src->dim * sizeof(mpq_lc_info));
    }
    if (src->lr_inf) {
        ILL_SAFE_MALLOC(dest->lr_inf, src->dim, mpq_lr_info);
        memcpy(dest->lr_inf, src->lr_inf, src->dim * sizeof(mpq_lr_info));
    }
    if (src->er_inf) {
        ILL_SAFE_MALLOC(dest->er_inf, src->etamax, mpq_er_info);
        memcpy(dest->er_inf, src->er_inf, src->etamax * sizeof(mpq_er_info));
    }
    ...
    if (src->uccoef) {
        dest->uccoef = mpq_EGlpNumAllocArray(src->uc_space);
        if (!dest->uccoef) { rval = 1; goto CLEANUP; }
        for (i = 0; i < src->uc_space; i++) {
            mpq_EGlpNumCopy(dest->uccoef[i], src->uccoef[i]);
        }
    }
    ...
    if (src->urcoef) {
        dest->urcoef = mpq_EGlpNumAllocArray(src->ur_space);
        if (!dest->urcoef) { rval = 1; goto CLEANUP; }
        for (i = 0; i < src->ur_space; i++) {
            mpq_EGlpNumCopy(dest->urcoef[i], src->urcoef[i]);
        }
    }
    ...
    if (src->lccoef) {
        dest->lccoef = mpq_EGlpNumAllocArray(src->lc_space);
        if (!dest->lccoef) { rval = 1; goto CLEANUP; }
        for (i = 0; i < src->lc_space; i++) {
            mpq_EGlpNumCopy(dest->lccoef[i], src->lccoef[i]);
        }
    }
	...
	if (src->lrcoef) {
		...
		dest->lrcoef = mpq_EGlpNumAllocArray(lr_nzcnt);
		if (!dest->lrcoef) { rval = 1; goto CLEANUP; }
		for (i = 0; i < lr_nzcnt; i++) {
			mpq_EGlpNumCopy(dest->lrcoef[i], src->lrcoef[i]);
		}
	}
    ...
    if (src->ercoef) {
        dest->ercoef = mpq_EGlpNumAllocArray(src->er_space);
        if (!dest->ercoef) { rval = 1; goto CLEANUP; }
        for (i = 0; i < src->er_space; i++) {
            mpq_EGlpNumCopy(dest->ercoef[i], src->ercoef[i]);
        }
    }
    ...
    if (src->dmat) {
        ...
        dest->dmat = mpq_EGlpNumAllocArray(dsize);
        if (!dest->dmat) { rval = 1; goto CLEANUP; }
        for (i = 0; i < dsize; i++) {
            mpq_EGlpNumCopy(dest->dmat[i], src->dmat[i]);
        }
    }

    // svector xtmp
    rval = mpq_ILLsvector_alloc(&dest->xtmp, src->dim);
    ...
    if (src->xtmp.nzcnt > 0) {
        ...
        for (i = 0; i < src->xtmp.nzcnt; i++) {
            mpq_EGlpNumCopy(dest->xtmp.coef[i], src->xtmp.coef[i]);
        }
    }
...
CLEANUP:
    ...
    mpq_ILLfactor_free_factor_work(dest);
