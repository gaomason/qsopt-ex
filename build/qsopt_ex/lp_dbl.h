...
/* RCSINFO $Id: lp_dbl.h,v 1.2 2003/11/05 16:57:39 meven Exp $ */
#ifndef dbl_LP_H
#define dbl_LP_H
...
extern int dbl_ILLread_lp (
	dbl_qsline_reader * file,
	const char *fname,
	dbl_rawlpdata * lp);
extern int dbl_ILLwrite_lp (
	dbl_ILLlpdata * l,
	dbl_qserror_collector * collector);

			/* write using current lp->reporter */
extern int dbl_ILLis_lp_name_char (
	int c,
	int pos);

extern int dbl_ILLread_constraint_name (
	dbl_ILLread_lp_state * state,
	char **rowname);
extern int dbl_ILLread_one_constraint (
	dbl_ILLread_lp_state * state,
	const char *rowname,
	dbl_rawlpdata * lp,
	int allowNewColsAddRow);
extern int dbl_ILLread_constraint_expr (
	dbl_ILLread_lp_state * state,
	dbl_rawlpdata * lp,
	int rowind,
	int allowNew);
