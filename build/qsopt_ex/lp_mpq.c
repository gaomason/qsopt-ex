...
/*  EXPORTED FUNCTIONS                                                      */
/*                                                                          */
/*        int mpq_ILLwrite_lp (EGioFile_t *out, mpq_ILLlpdata *lp)                  */
/*        int mpq_ILLread_lp (mpq_qsline_reader f, const char *fname, mpq_rawlpdata *lp)*/
	...
	mpq_qserror_collector * collector)
{
	...
	/* Note, mpq_ILLlp_rows_init returns cols ordered by structmap, so we may use 
	...
	for (ri = 0, var = 0; ri < lp->nstruct; ri++)
	{
		...
		if (mpq_EGlpNumIsNeqqZero (lp->obj[i]))
		{
			...
			if ((line->total >= LINE_LEN) && (var >= 4))
			{
				...
				while (k < lp->nstruct)
				{
					if (mpq_EGlpNumIsLessZero (lp->obj[lp->structmap[k]]))
					{
						break;
					}
					else
					{
						if (mpq_EGlpNumIsGreatZero (lp->obj[lp->structmap[k]]))
						{
							mpq_ILLwrite_lp_state_append (line, " +");
							break;
						}
		...
		if (colInRow[i] == row)
		{
			if (mpq_EGlpNumIsNeqqZero (colCoef[i]))
			{
				...
				if (line->total >= LINE_LEN)
				{
					mpq_ILLprint_report (lp, "%s\n", line->buf);
					mpq_ILLwrite_lp_state_start (line);
					if ((!firstVar) && !mpq_EGlpNumIsLessZero (*coef))
					{
						mpq_ILLwrite_lp_state_append (line, " +");
					}
					var = 0;
 ...
 */
	...
	rval = mpq_ILLread_lp_state_init (state, file, fname, 0);
	CHECKRVALG (rval, CLEANUP);
...
int mpq_ILLread_constraint_expr (
	mpq_ILLread_lp_state * state,
	mpq_rawlpdata * lp,
	int rowind,
	int allowNew)
...
int mpq_ILLread_constraint_name (
	mpq_ILLread_lp_state * state,
	char **rowname)
{
	int rval = 0;
...
int mpq_ILLread_one_constraint (
	mpq_ILLread_lp_state * state,
	const char *rowname,
	mpq_rawlpdata * lp,
	int allowNewCols)
