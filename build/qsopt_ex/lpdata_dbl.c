	...
	int isMps)
{
	...
	p = dbl_QScreate_prob (fname, QS_MIN);
	...
	if (isMps != 0)
	{
		rval = dbl_ILLread_mps (file, fname, &rawlp);
	}
	else
	{
		rval = dbl_ILLread_lp (file, fname, &rawlp);
	}
	...
	dbl_ILLlpdata * lp)
{
	if (lp)
	{
		...
		ILLstring_reporter_init(
			&lp->reporter, dbl_ILLlpdata_log, NULL);
	}
}
