...
/*  RCS_INFO = "$RCSfile: rawlp_dbl.h,v $ $Revision: 1.3 $ $Date: 2003/11/05 16:57:39 $"; */
...
dbl_colptr;
extern dbl_colptr *dbl_ILLcolptralloc (
	ILLptrworld * p);
...
extern void dbl_ILLinit_rawlpdata (
	dbl_rawlpdata * lp,
	dbl_qserror_collector * collector);
extern void dbl_ILLfree_rawlpdata (
	dbl_rawlpdata * lp);
extern void dbl_ILLraw_clear_matrix (
	dbl_rawlpdata * lp);

extern const char *dbl_ILLraw_rowname (
	dbl_rawlpdata * lp,
	int i);
extern const char *dbl_ILLraw_colname (
	dbl_rawlpdata * lp,
	int i);

extern int dbl_ILLraw_add_col (
	dbl_rawlpdata * lp,
	const char *name,
	int intmarker);
extern int dbl_ILLraw_add_row (
	dbl_rawlpdata * lp,
	const char *name,
	int sense,
	const double rhs);

extern int dbl_ILLraw_add_col_coef (
	dbl_rawlpdata * lp,
	int colind,
	int rowind,
	double coef);

extern int dbl_ILLraw_init_ranges (
	dbl_rawlpdata * lp);
extern int dbl_ILLraw_init_rhs (
	dbl_rawlpdata * lp);

extern int dbl_ILLraw_add_ranges_coef (
	dbl_rawlpdata * lp,
	int rowind,
	double coef);


extern int dbl_ILLraw_add_sos (
	dbl_rawlpdata * lp,
	int sos_type);

																								/* add empty set with type */
extern int dbl_ILLraw_add_sos_member (
	dbl_rawlpdata * lp,
	int colind);

																								/* add col to last set */
extern int dbl_ILLraw_is_mem_other_sos (
	dbl_rawlpdata * lp,
	int colind);

extern int dbl_ILLraw_set_rhs_name (
	dbl_rawlpdata * lp,
	const char *name,
	int *skip);
extern int dbl_ILLraw_set_bounds_name (
	dbl_rawlpdata * lp,
	const char *name,
	int *skip);
extern int dbl_ILLraw_set_ranges_name (
	dbl_rawlpdata * lp,
	const char *name,
	int *skip);
extern void dbl_ILLprint_rawlpdata (
	dbl_rawlpdata * lp);

extern char *dbl_ILLraw_unique_name (
	ILLsymboltab * tab,
	char *prefix,
	int i);
extern int dbl_ILLraw_fill_in_rownames (
	dbl_rawlpdata * lp);

extern int dbl_ILLraw_init_bounds (
	dbl_rawlpdata * lp);

extern const char *dbl_ILLraw_set_lowerBound (
	dbl_rawlpdata * lp,
	int i,
	double bnd);
extern const char *dbl_ILLraw_set_upperBound (
	dbl_rawlpdata * lp,
	int i,
	double bnd);
extern const char *dbl_ILLraw_set_fixedBound (
	dbl_rawlpdata * lp,
	int i,
	double bnd);
extern const char *dbl_ILLraw_set_binaryBound (
	dbl_rawlpdata * lp,
	int i);
extern const char *dbl_ILLraw_set_unbound (
	dbl_rawlpdata * lp,
	int colind);
extern int dbl_ILLraw_fill_in_bounds (
	dbl_rawlpdata * lp);

extern int dbl_ILLraw_first_nondefault_bound (
	dbl_ILLlpdata * lp);
extern int dbl_ILLraw_default_lower (
	dbl_ILLlpdata * lp,
	int i);
extern int dbl_ILLraw_default_upper (
	dbl_ILLlpdata * lp,
	int i,
	int ri);

extern int dbl_ILLrawlpdata_to_lpdata (
	dbl_rawlpdata * raw,
	dbl_ILLlpdata * lp);

extern int dbl_ILLdata_error (
	dbl_qserror_collector * collector,
	const char *format,
	...);
extern void dbl_ILLdata_warn (
	dbl_qserror_collector * collector,
	const char *format,
	...);
