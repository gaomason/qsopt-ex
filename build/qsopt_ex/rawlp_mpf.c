...
ILL_PTRWORLD_LISTFREE_ROUTINE (mpf_colptr, colptr_listfree, colptrfree)
ILL_PTRWORLD_LEAKS_ROUTINE (mpf_colptr, colptr_check_leaks, this_val, int)
...
int mpf_ILLraw_add_ranges_coef (
	mpf_rawlpdata * lp,
	int rowind,
	mpf_t coef)
{
...
int mpf_ILLraw_is_mem_other_sos (
	mpf_rawlpdata * lp,
	int colind)
{
...
const char *mpf_ILLraw_set_lowerBound (
	mpf_rawlpdata * lp,
	int i,
	mpf_t bnd)
{
...
const char *mpf_ILLraw_set_upperBound (
	mpf_rawlpdata * lp,
	int i,
	mpf_t bnd)
{
	...
	lp->ubind[i] = (char) 1;
	if (!mpf_EGlpNumIsNeqqZero (lp->lower[i]) &&
			!mpf_EGlpNumIsNeqqZero (bnd))
	{
...
const char *mpf_ILLraw_set_fixedBound (
	mpf_rawlpdata * lp,
	int i,
	mpf_t bnd)
{
...
const char *mpf_ILLraw_set_unbound (
	mpf_rawlpdata * lp,
	int i)
{
	ILL_FAILtrue_no_rval (i >= lp->ncols, "proper colind");
...
const char *mpf_ILLraw_set_binaryBound (
	mpf_rawlpdata * lp,
	int i)
{
	ILL_FAILtrue_no_rval (i >= lp->ncols, "proper colind");
	...
	mpf_rawlpdata * lp)
{
	...
	for (i = 0; i < lp->ncols; i++)
	{
		if (!lp->lbind[i])
		{
			if (lp->ubind[i] && mpf_EGlpNumIsLessZero (lp->upper[i]))
			{
				mpf_EGlpNumCopy (lp->lower[i], mpf_ILL_MINDOUBLE);
			}
		}
...
int mpf_ILLraw_first_nondefault_bound (
	mpf_ILLlpdata * lp)
{
	int ri = lp->nstruct, i;
	...
	{
		i = lp->structmap[ri];
		if (!mpf_ILLraw_default_lower (lp, i) || !mpf_ILLraw_default_upper (lp, i, ri))
			break;
	}
CLEANUP:
	return ri;
	...
	int i)
{
	...
	if (!mpf_EGlpNumIsNeqqZero (lp->lower[i]) &&
			!mpf_EGlpNumIsLessZero (lp->upper[i]))
	{
		return 1;
	}
...
int mpf_ILLraw_fill_in_rownames (
	mpf_rawlpdata * lp)
{
	int i, rval = 0;
	...
	int *colindex)
{
	...
	for (i = 0; i < raw->ncols; i++)
	{
		...
		if (ci != -1)
		{
			...
			ILL_UTIL_STR (lp->colnames[ci], mpf_ILLraw_colname (raw, i));
	...
	int *rowindex)
{
	...
	if (lp->nrows > 0)
	{
		...
		safeRegister (&lp->rowtab, mpf_ILLraw_rowname (raw, raw->objindex), 0);
		...
		for (i = 0; i < raw->nrows; i++)
		{
			...
			if (i == raw->refrowind)
			{
				ILL_UTIL_STR (lp->refrowname, mpf_ILLraw_rowname (raw, i));
				lp->refind = ri;
			}
			if (raw->rowsense[i] != 'N')
			{
				...
				ILL_UTIL_STR (lp->rownames[ri], mpf_ILLraw_rowname (raw, i));
			...
			else if (i == objind)
			{
				...
				ILL_UTIL_STR (lp->objname, mpf_ILLraw_rowname (raw, i));
			}
			else
			{
				/* unused 'N' row */
	...
	int *rowindex)
{
	...
	for (cp = raw->ranges; cp; cp = cp->next)
	{
		...
		case 'E':
			lp->sense[ri] = 'R';
			if (!mpf_EGlpNumIsLessZero (cp->coef))
			{
	...
	int *colindex)
{
	...
	/* build sos info */
	/* see comment in lpdata_mpf.h about mpf_ILLlpdata's sos and is_sos_mem 
...
int mpf_ILLrawlpdata_to_lpdata (
	mpf_rawlpdata * raw,
	mpf_ILLlpdata * lp)
{
	int rval = 0;
...
int mpf_ILLraw_set_bounds_name (
	mpf_rawlpdata * lp,
	const char *name,
	int *skip)
{
...
int mpf_ILLraw_set_ranges_name (
	mpf_rawlpdata * lp,
	const char *name,
	int *skip)
{
	...
	if (lp)
	{
		...
		if (lp->ncols > 0)
		{
			...
			for (i = 0; i < lp->ncols; i++)
			{
				...
				{
					printf ("%s: ", mpf_ILLraw_rowname (lp, cp->this_val));
					printf ("%c ", (mpf_EGlpNumIsLessZero (cp->coef)) ? '-' : '+');
		...
		if (lp->rangesname)
		{
			...
			{
				printf ("(%s, %f) ", mpf_ILLraw_rowname (lp, cp->this_val),
