...
int dbl_ILLread_lp_state_next_line (
	dbl_ILLread_lp_state * state)
{
	char *slash;
...
int dbl_ILLread_lp_state_skip_blanks (
	dbl_ILLread_lp_state * state,
	int wrapLines)
{
	while (1)
...
int dbl_ILLread_lp_state_next_field_on_line (
	dbl_ILLread_lp_state * state)
{
	return next_field (state, 0);
}

int dbl_ILLread_lp_state_next_field (
	dbl_ILLread_lp_state * state)
{
	return next_field (state, 1);
}

void dbl_ILLread_lp_state_prev_field (
	dbl_ILLread_lp_state * state)
{
...
int dbl_ILLread_lp_state_next_var (
	dbl_ILLread_lp_state * state)
{
	char *p;
	int var_len, i;
...
int dbl_ILLread_lp_state_bad_keyword (
	dbl_ILLread_lp_state * state)
{
	if (!state->fieldOnFirstCol)
	{
...
int dbl_ILLtest_lp_state_keyword (
	dbl_ILLread_lp_state * state,
	const char *kwd[])
{
	int i = 0;
...
int dbl_ILLread_lp_state_keyword (
	dbl_ILLread_lp_state * state,
	const char *kwd[])
{
...
int dbl_ILLread_lp_state_colon (
	dbl_ILLread_lp_state * state)
{
...
int dbl_ILLread_lp_state_has_colon (
	dbl_ILLread_lp_state * state)
{
	char *pp;
...
int dbl_ILLread_lp_state_next_constraint (
	dbl_ILLread_lp_state * state)
{
	int rval;
	...
	{
		rval = dbl_ILLtest_lp_state_keyword (state, all_keyword);
...
int dbl_ILLtest_lp_state_next_is (
	dbl_ILLread_lp_state * state,
	const char *str)
{
	dbl_ILLread_lp_state_skip_blanks (state, 0);
...
int dbl_ILLread_lp_state_value (
	dbl_ILLread_lp_state * state,
	double * coef)
{
	int len = 0;
...
int dbl_ILLread_lp_state_possible_coef (
	dbl_ILLread_lp_state * state,
	double * coef,
...
int dbl_ILLread_lp_state_possible_bound_value (
	dbl_ILLread_lp_state * state)
{
	double sign;
	int len = 0;
	...
	if (len > 0)
	{
		...
		{
			if (dbl_EGlpNumIsLessZero (sign))
				dbl_EGlpNumCopy (state->bound_val, dbl_ILL_MINDOUBLE);
			else if (dbl_EGlpNumIsGreatZero (sign))
...
int dbl_ILLtest_lp_state_sense (
	dbl_ILLread_lp_state * state,
	int all)
{
	char c;
...
void dbl_ILLtest_lp_state_bound_sense (
	dbl_ILLread_lp_state * state)
{
	(void) dbl_ILLtest_lp_state_sense (state, 0);
}

int dbl_ILLread_lp_state_sense (
	dbl_ILLread_lp_state * state)
{
	...
	dbl_ILLread_lp_state * state)
{
	...
	else
	{
		...
		else
		{
			...
			QSlog("%c", '"');
			for (; !dbl_ILL_ISBLANK (p) && !END_LINE (p); p++)
			{
				QSlog("%c", *p);
			}
			QSlog("\"");
