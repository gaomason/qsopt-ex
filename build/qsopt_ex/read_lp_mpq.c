...
int mpq_ILLread_lp_state_next_line (
	mpq_ILLread_lp_state * state)
{
	char *slash;
...
int mpq_ILLread_lp_state_skip_blanks (
	mpq_ILLread_lp_state * state,
	int wrapLines)
{
	while (1)
...
int mpq_ILLread_lp_state_next_field_on_line (
	mpq_ILLread_lp_state * state)
{
	return next_field (state, 0);
}

int mpq_ILLread_lp_state_next_field (
	mpq_ILLread_lp_state * state)
{
	return next_field (state, 1);
}

void mpq_ILLread_lp_state_prev_field (
	mpq_ILLread_lp_state * state)
{
...
int mpq_ILLread_lp_state_next_var (
	mpq_ILLread_lp_state * state)
{
	char *p;
	int var_len, i;
...
int mpq_ILLread_lp_state_bad_keyword (
	mpq_ILLread_lp_state * state)
{
	if (!state->fieldOnFirstCol)
	{
...
int mpq_ILLtest_lp_state_keyword (
	mpq_ILLread_lp_state * state,
	const char *kwd[])
{
	int i = 0;
...
int mpq_ILLread_lp_state_keyword (
	mpq_ILLread_lp_state * state,
	const char *kwd[])
{
...
int mpq_ILLread_lp_state_colon (
	mpq_ILLread_lp_state * state)
{
...
int mpq_ILLread_lp_state_has_colon (
	mpq_ILLread_lp_state * state)
{
	char *pp;
...
int mpq_ILLread_lp_state_next_constraint (
	mpq_ILLread_lp_state * state)
{
	int rval;
	...
	{
		rval = mpq_ILLtest_lp_state_keyword (state, all_keyword);
...
int mpq_ILLtest_lp_state_next_is (
	mpq_ILLread_lp_state * state,
	const char *str)
{
	mpq_ILLread_lp_state_skip_blanks (state, 0);
...
int mpq_ILLread_lp_state_value (
	mpq_ILLread_lp_state * state,
	mpq_t * coef)
{
	int len = 0;
...
int mpq_ILLread_lp_state_possible_coef (
	mpq_ILLread_lp_state * state,
	mpq_t * coef,
	const mpq_t defValue)
{
...
int mpq_ILLread_lp_state_possible_bound_value (
	mpq_ILLread_lp_state * state)
{
	mpq_t sign;
	int len = 0;
	...
	if (len > 0)
	{
		...
		{
			if (mpq_EGlpNumIsLessZero (sign))
				mpq_EGlpNumCopy (state->bound_val, mpq_ILL_MINDOUBLE);
			else if (mpq_EGlpNumIsGreatZero (sign))
...
int mpq_ILLtest_lp_state_sense (
	mpq_ILLread_lp_state * state,
	int all)
{
	char c;
...
void mpq_ILLtest_lp_state_bound_sense (
	mpq_ILLread_lp_state * state)
{
	(void) mpq_ILLtest_lp_state_sense (state, 0);
}

int mpq_ILLread_lp_state_sense (
	mpq_ILLread_lp_state * state)
{
	...
	mpq_ILLread_lp_state * state)
{
	...
	else
	{
		...
		else
		{
			...
			QSlog("%c", '"');
			for (; !mpq_ILL_ISBLANK (p) && !END_LINE (p); p++)
			{
				QSlog("%c", *p);
			}
			QSlog("\"");
