...
#include "read_lp_dbl.h"						/* for dbl_ILLget_value */
...
CLEANUP:
	ILL_RESULT (rval, "dbl_ILLmps_state_init");
}

int dbl_ILLmps_next_line (
	dbl_ILLread_mps_state * state)
{
	int rval = 0;
	...
	state->line[0] = '\0';
	state->p = 0;
	while (dbl_ILLline_reader_get (state->line, ILL_namebufsize - 2, state->file)
				 != 0)
	{
		state->line_num++;
		state->key[0] = '\0';
	...
	double * coef)
{
	...
	ILL_RESULT (!(len > 0), "dbl_ILLmps_next_coef");
}

int dbl_ILLmps_next_bound (
	dbl_ILLread_mps_state * state,
	double * coef)
{
	int len = 0, sign = 1;
	...
	if (!mps_skip_comment (state))
	{
		...
		if (len > 1)
		{
			...
			{
				if (sign == 1)
					dbl_EGlpNumCopy (*coef, dbl_ILL_MAXDOUBLE);
				else
					dbl_EGlpNumCopy (*coef, dbl_ILL_MINDOUBLE);
				state->field_num++;
				ILL_RESULT (0, "dbl_ILLmps_next_bound");
			}
		}
		...
		{
			ILL_RESULT (1, "dbl_ILLmps_next_bound");	/* no coef found */
		}
	}
	ILL_RETURN (1, "dbl_ILLmps_next_bound");
}
...
void dbl_ILLmps_check_end_of_line (
	dbl_ILLread_mps_state * state)
{
	if (!mps_skip_comment (state))
	{
		if (!END_LINE (state->p))
		{
			dbl_ILLmps_warn (state, "Extra fields on line.");
		}
	}
}

void dbl_ILLmps_set_end_of_line (
	dbl_ILLread_mps_state * state)
{
	*state->p = '\n';
}

int dbl_ILLmps_set_section (
	dbl_ILLread_mps_state * state,
	const ILLmps_section sec)
{
	...
	{
		rval = dbl_ILLmps_error (state, "Two %s sections.\n", dbl_ILLmps_section_name[sec]);
	}
	state->section[sec]++;
	state->active = sec;
CLEANUP:
	ILL_RESULT (rval, "dbl_ILLmps_set_section");
}

int dbl_ILLmps_int_sos_mode (
	dbl_ILLread_mps_state * state)
{
	if (!strcmp (state->field, "'INTORG'"))
	{
		if (state->intvar)
		{
			return !dbl_ILLmps_error (state, "'INTEND' expected.\n");
		}
		else
		{
			state->intvar = 1;
			ILL_RESULT (0, "dbl_ILLmps_int_sos_mode");
		}
	}
	if (!strcmp (state->field, "'INTEND'"))
	{
		...
		{
			return !dbl_ILLmps_error (state, "'INTORG' expected.\n");
		}
	}
	if (!strcmp (state->field, "'SOSORG'"))
	{
		if (state->sosvar)
		{
			return !dbl_ILLmps_error (state, "'SOSEND' expected.\n");
		}
		else
		{
			state->sosvar = 1;
	...
	if (!strcmp (state->field, "'SOSEND'"))
	{
		...
		{
			return !dbl_ILLmps_error (state, "'SOSORG' expected.\n");
		}
	}
	return dbl_ILLmps_error (state, "%s is not a MARKER field.\n", state->field);
}

const char *dbl_ILLmps_possibly_blank_name (
	const char *field,
	dbl_ILLread_mps_state * state,
	ILLsymboltab * tab)
{
...
int dbl_ILLmps_empty_key (
	dbl_ILLread_mps_state * state)
{
	return state->key[0] == '\0';
}

int dbl_ILLmps_empty_field (
	dbl_ILLread_mps_state * state)
{
	...
	...)
{
	...
	/* ILL_RESULT(1, "dbl_ILLmps_error"); */
	return 1;
}

void dbl_ILLmps_warn (
	dbl_ILLread_mps_state * state,
	const char *format,
	...)
{
