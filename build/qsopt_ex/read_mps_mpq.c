...
#include "read_lp_mpq.h"						/* for mpq_ILLget_value */
...
void mpq_ILLmps_check_end_of_line (
	mpq_ILLread_mps_state * state)
{
	if (!mps_skip_comment (state))
	{
...
void mpq_ILLmps_set_end_of_line (
	mpq_ILLread_mps_state * state)
{
	*state->p = '\n';
}

int mpq_ILLmps_set_section (
	mpq_ILLread_mps_state * state,
...
const char *mpq_ILLmps_possibly_blank_name (
	const char *field,
	mpq_ILLread_mps_state * state,
	ILLsymboltab * tab)
{
...
int mpq_ILLmps_empty_key (
	mpq_ILLread_mps_state * state)
{
	return state->key[0] == '\0';
}

int mpq_ILLmps_empty_field (
	mpq_ILLread_mps_state * state)
{
