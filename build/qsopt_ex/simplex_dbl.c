...
int dbl_ILLsimplex (dbl_lpinfo * lp, int algorithm, dbl_ILLlp_basis * B,
	dbl_price_info * pinf, int *status, int sdisplay, itcnt_t*itcnt) {
	int phase = -1;
	int singular = -1;
	int rval = 0;
	int new_price = -1;
...
static int terminate_simplex (dbl_lpinfo * lp, int phase, dbl_iter_info * it)
{
	int rval = 0;
	int sphase;
	dbl_feas_info fi;
...
static void monitor_iter (dbl_lpinfo * lp, dbl_price_info * p, dbl_iter_info * it, int phase) {
	double print_val;
...
static int primal_phaseI_step (dbl_lpinfo * lp, dbl_price_info * pinf,
	dbl_svector * updz, dbl_svector * wz, dbl_iter_info * it) {
	int rval = 0;
	int singular = 0;
	int refactor = 0;
...
static void get_current_stat (dbl_lp_status_info * p, int algorithm, int *bstat) {
	if (p->optimal)
		*bstat = OPTIMAL;
...
static int report_value (dbl_lpinfo * lp, dbl_iter_info * it, const char *value_name, double value) {
	int rval = 0;

	// dbl_ILLlib_writebasis(lp, 0, "basis_everystep");
