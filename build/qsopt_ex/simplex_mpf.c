...
#include "qsopt_mpf.h"
#include "lib_mpf.h"								/* for mpf_ILLlib_writebasis */
#include "lp_mpf.h"									/* for mpf_ILLwrite_lp */
...
void mpf_ILLsimplex_init_lpinfo (
	mpf_lpinfo * lp)
{
...
void mpf_ILLsimplex_free_lpinfo (
	mpf_lpinfo * lp)
{
	if (lp)
	{
...
void mpf_ILLsimplex_load_lpinfo (
	mpf_ILLlpdata * qslp,
	mpf_lpinfo * lp)
{
	lp->basisid = -1;
	...
	mpf_lpinfo * lp)
{
	...
	if (mpf_EGlpNumIsNeqqZero (lp->tol->ip_tol))
	{
		...
		MESSAGE (VERBOSE_LEVEL, "ip_tol %lg", mpf_EGlpNumToLf (lp->tol->ip_tol));
		MESSAGE (VERBOSE_LEVEL, "eps %lg", mpf_EGlpNumToLf (mpf_epsLpNum));
		MESSAGE (VERBOSE_LEVEL, "mpf_PFEAS_TOLER %lg", mpf_EGlpNumToLf (mpf_PFEAS_TOLER));
#endif
	...
	{
#if VERBOSE_LEVEL <= DEBUG
		MESSAGE (VERBOSE_LEVEL, "id_tol %lg", mpf_EGlpNumToLf (lp->tol->id_tol));
#endif
		mpf_EGlpNumDivUiTo (lp->tol->id_tol, 2UL);
	}
}

void mpf_init_internal_lpinfo (
	mpf_lpinfo * lp)
{
	int rval = 0;
	...
	mpf_lpinfo * lp)
{
	...
	{
		QSlog("mpf_build_internal_lpinfo");
		rval = 1;
		goto CLEANUP;
	}
...
int mpf_ILLsimplex_retest_psolution (
	mpf_lpinfo * lp,
	mpf_price_info * p,
	int phase,
	mpf_feas_info * fi)
...
int mpf_ILLsimplex_retest_dsolution (
	mpf_lpinfo * lp,
	mpf_price_info * p,
	int phase,
	mpf_feas_info * fi)
...
int mpf_ILLsimplex_infcertificate (
	mpf_lpinfo * lp,
	mpf_t * pi)
{
	int i, col, nz;
	...
	for (i = 0; i < lp->nrows; i++)
	{
		if (sense[i] == 'G' && mpf_EGlpNumIsLessZero (pi[i]))
			mpf_EGlpNumZero (pi[i]);
		if (sense[i] == 'L' && mpf_EGlpNumIsGreatZero (pi[i]))
			mpf_EGlpNumZero (pi[i]);
	}
	return 0;
}
	...
	mpf_t * pi)
{
	...
	for (i = 0; i < lp->nrows; i++)
	{
		if (lp->O->sense[i] == 'G' && mpf_EGlpNumIsLessZero (pi[i]))
			QSlog("compl");
		if (lp->O->sense[i] == 'L' && mpf_EGlpNumIsGreatZero (pi[i]))
			QSlog("compll");
	}
...
int mpf_ILLsimplex (mpf_lpinfo * lp, int algorithm, mpf_ILLlp_basis * B,
	mpf_price_info * pinf, int *status, int sdisplay, itcnt_t*itcnt) {
	int phase = -1;
	int singular = -1;
	int rval = 0;
	int new_price = -1;
	...
	if (it.sdisplay)
	{
		...
		while (i--)
			nonzero += lp->matcnt[i];
		sprintf (buffer, "starting mpf_ILLsimplex on %s...", lp->O->probname);
	...
	if (B != 0)
	{
		...
		{
			QSlog("Unknown algorithm %d in mpf_ILLsimplex", it.algorithm);
			rval = 1;
			ILL_CLEANUP;
		}
	}
...
TERMINATE:
	...
	if (it.sdisplay)
	{
		QSlog("completed mpf_ILLsimplex");
	...
	if (status)
	{
		...
		{
			QSlog("unknown solution status in mpf_ILLsimplex %d",
									it.solstatus);
			rval = 1;
			CHECKRVALG (rval, CLEANUP);
		}
   ...
   /* end update */
	if (it.sdisplay)
	{
		...
		{
		case OPTIMAL:
			QSlog("opt = %f", mpf_EGlpNumToLf (lp->objval));
			break;
		...
		case PRIMAL_FEASIBLE:
			QSlog("primal obj = %f", mpf_EGlpNumToLf (lp->pobjval));
			break;
		...
		case DUAL_UNBOUNDED:
			...
			break;
		case DUAL_FEASIBLE:
			QSlog("dual obj = %f", mpf_EGlpNumToLf (lp->dobjval));
			break;
		}
...
static int terminate_simplex (mpf_lpinfo * lp, int phase, mpf_iter_info * it)
{
	int rval = 0;
	int sphase;
	mpf_feas_info fi;
	...
	mpf_t prevobj)
{
	...
	if (!mpf_EGlpNumIsNeqZero (denom, mpf_PROGRESS_THRESH))
	{
...
static void monitor_iter (mpf_lpinfo * lp, mpf_price_info * p, mpf_iter_info * it, int phase) {
	mpf_t print_val;
	...
	case PRIMAL_PHASEI:
		...
		mpf_EGlpNumAddTo (fi.totinfeas, lp->pinfeas);
		strcpy (print_str, "primal infeas");
		if (mpf_EGlpNumIsLessZero (lp->pinfeas) &&
				(mpf_EGlpNumIsNeqZero (lp->pinfeas, mpf_oneLpNum)))
		{
			/*QSlog("Negative Infeasibility! Imposible %lg %la, iter %d",
			 * mpf_EGlpNumToLf (print_val), mpf_EGlpNumToLf (print_val), it->itercnt);
			 */
			//exit(1);
		}
		break;
	...
	case DUAL_PHASEI:
		...
		mpf_EGlpNumAddTo (fi.totinfeas, lp->dinfeas);
	...
	mpf_EGlpNumAddUiTo (fi.totinfeas, 1000);
	if (mpf_EGlpNumIsLessZero (fi.totinfeas))
	{
		...
		if (mpf_EGlpNumIsLess (it->objtol, print_val))
		{
			...
			if (fi.dstatus == DUAL_FEASIBLE)
			{
				...
				if (mpf_EGlpNumIsLess (lp->objbound, lp->dobjval))
				{
					...
					/*if (it->sdisplay) */
					{
						QSlog("bound reached %lf %lf", mpf_EGlpNumToLf (lp->objbound),
										mpf_EGlpNumToLf (lp->dobjval));
					}
				}
				else
...
static int primal_phaseI_step (mpf_lpinfo * lp, mpf_price_info * pinf,
	mpf_svector * updz, mpf_svector * wz, mpf_iter_info * it) {
	int rval = 0;
	int singular = 0;
	int refactor = 0;
	...
	mpf_iter_info * it)
{
	...
	if (pr.price_stat == PRICE_OPTIMAL)
	{
		...
		if (it->sdisplay > 1)
		{
			//ILL_IFTRACE("\t%s:%s:%d\n",__func__,__FILE__,__LINE__);
			QSlog("problem seemingly solved");
			QSlog("seemingly dual opt = %f", mpf_EGlpNumToLf (lp->dobjval));
			QSlog("retesting soln");
		}
	...
	else if (rs.ratio_stat == RATIO_BCHANGE)
	{
		...
		mpf_ILLfct_update_dpII_prices (lp, pinf, &(lp->srhs), &(lp->ssoln), /*rs.eindex,*/
...
static void get_current_stat (mpf_lp_status_info * p, int algorithm, int *bstat) {
	if (p->optimal)
		*bstat = OPTIMAL;
	...
	int *basis_mod)
{
	...
	*basis_mod = 0;
	...
	/* QSlog("Forcing vars into basis in mpf_ILLsimplex_pivotin"); */
	mpf_ILLsvector_init (&wz);
	rval = mpf_ILLsvector_alloc (&wz, lp->nrows);
	CHECKRVALG (rval, CLEANUP);
	mpf_ILLsvector_init (&updz);
...
static int report_value (mpf_lpinfo * lp, mpf_iter_info * it, const char *value_name, mpf_t value) {
	int rval = 0;

	// mpf_ILLlib_writebasis(lp, 0, "basis_everystep");

	if (it->sdisplay && it->itercnt % lp->iterskip == 0) {
		char buffer[1024];

		snprintf (buffer, (size_t) 1023, "(%d): %s = %10.7lf", it->itercnt,
							value_name, mpf_EGlpNumToLf (value));
