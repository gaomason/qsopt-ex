...
int mpq_ILLsimplex (mpq_lpinfo * lp, int algorithm, mpq_ILLlp_basis * B,
	mpq_price_info * pinf, int *status, int sdisplay, itcnt_t*itcnt) {
	int phase = -1;
	int singular = -1;
	int rval = 0;
	int new_price = -1;
...
static int terminate_simplex (mpq_lpinfo * lp, int phase, mpq_iter_info * it)
{
	int rval = 0;
	int sphase;
	mpq_feas_info fi;
...
static void monitor_iter (mpq_lpinfo * lp, mpq_price_info * p, mpq_iter_info * it, int phase) {
	mpq_t print_val;
...
static int primal_phaseI_step (mpq_lpinfo * lp, mpq_price_info * pinf,
	mpq_svector * updz, mpq_svector * wz, mpq_iter_info * it) {
	int rval = 0;
	int singular = 0;
	int refactor = 0;
...
static void get_current_stat (mpq_lp_status_info * p, int algorithm, int *bstat) {
	if (p->optimal)
		*bstat = OPTIMAL;
...
static int report_value (mpq_lpinfo * lp, mpq_iter_info * it, const char *value_name, mpq_t value) {
	int rval = 0;

	// mpq_ILLlib_writebasis(lp, 0, "basis_everystep");
