	...
	if (p_mpq->cached_lu == 0) 
	{
		...
		while (!refactor) {
			...
			if (!changed) {
				...
				mpf_set_d(rhs.coef[0], 1.0);
				mpf_ILLfactor_btran(mpf_cached_lu, &rhs, &v);

				v_row = mpf_EGlpNumAllocArray(p_mpq->lp->nrows);
				if (!v_row) {
					mpf_ILLsvector_free(&rhs);
				...
				for (int i = 0; i < v.nzcnt; ++i) {
					mpf_set(v_row[v.indx[i]], v.coef[i]);
				}
				...
				for (int m = 0; m < mismatch_count; ++m) {
					...
					for (int j = 0; j < mcnt; ++j) {
						...
						mpf_set_q(coef, p_mpq->lp->matval[mbeg + j]);
						mpf_mul(aij, v_row[row], coef);
						mpf_add(pivot_dot, pivot_dot, aij);
					}
					mpf_abs(abs_dot, pivot_dot);
					if (mpf_cmp(abs_dot, pivot_eps) > 0) {
